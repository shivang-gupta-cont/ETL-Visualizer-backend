...
public class AuthService {
    ...
    public void register(RegisterRequestDTO registerRequestDTO) throws IllegalArgumentException {
        ...
        RegisterationReqs check2 = registerationReqsRepository.findByEmail(registerRequestDTO.getEmail()).orElse(null);
        if (check2 != null) {
            log.warn("Registration failed - request already pending for email: {}", registerRequestDTO.getEmail());
            throw new DuplicateResourceException("Request Already Made.");
        }

        if (usersRepository.findByUsername(registerRequestDTO.getUsername()).isPresent() ||
                registerationReqsRepository.findByUsername(registerRequestDTO.getUsername()).isPresent()) {
            log.warn("Registration failed - username already taken: {}", registerRequestDTO.getUsername());
            throw new DuplicateResourceException("Username already taken.");
        }
