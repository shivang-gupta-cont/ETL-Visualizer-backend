...
public class AuthService {
	...
	public LoginResponseDTO login(LoginRequestDTO loginRequestDTO) {
		...
		return new LoginResponseDTO(tokenString, user.getUsername(), user.getRole());
	}

	public ResponseEntity<HttpStatus> register(RegisterRequestDTO registerRequestDTO) throws IllegalArgumentException{
		Users check1 = usersRepository.findByUsername(registerRequestDTO.getUsername()).orElse(null);
		if(check1 != null) throw new IllegalArgumentException("User Already Exists");
		
		RegisterationReqs check2 = registerationReqsRepository.findByUsername(registerRequestDTO.getUsername()).orElse(null);
